...
multi_steps_wrapper <- function(dataset, grid_size, vector_of_predictors_chr, no_steps, no_fits, no_trees, cell_fraction, train_fraction, level_num, ID_exp, ID_run, addition)
{
  ...
  for (i in 1:no_steps)
  {
    ...
    #for debugging - see outfile arg below
    debug_outfile_name <- sprintf("worker_logfile_%s_%s_%s_%s%s", paste("exp", ID_exp, sep="_"), paste("run", ID_run, sep="_"), 
                                  paste("level", level_num, sep="_"), paste("step", i, sep="_"), ".txt")
    
    cl <- parallel::makePSOCKcluster(no_cores, outfile = file.path("output", "dengue_dataset", "predictor_importance_test", output_folder, 
                                                                   paste("exp", ID_exp, sep="_"), 
                                                                   paste("run", ID_run, sep="_"), debug_outfile_name))
    junk <- parallel::clusterCall(cl, ".libPaths", .libPaths())
    junk2 <- parallel::clusterEvalQ(cl, c(library(ranger), library(weights)))
    parallel::clusterExport(cl, "spatial.cv.rf", envir = .GlobalEnv)
    parallel::clusterExport(cl, c("grid_size", "no_fits", "no_trees", "cell_fraction", "train_fraction"), envir = environment())
    predictor_importance_test_output_local <- parallel::clusterApply(cl, combinations_of_predictors, 
