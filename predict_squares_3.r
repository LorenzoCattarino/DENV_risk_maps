...
out_path <- "output/predictions_world/boot_model_20km_cw"
...
write.csv(num_Inf_and_C_by_c, 
          "output/predictions_world/boot_model_20km_cw/total_infec_and_cases_by_country.csv",
          row.names = FALSE)
...
summary_table$scaling_factor <- factor(summary_table$scaling_factor, levels = c(1,0.7,0.3), labels = c(1,0.7,0.3))

phi_factor_levels <- c("Up to 2 infections", "Up to 4 infections (sym twice as infectious as asym)")

summary_table$phi_set_id <- factor(summary_table$phi_set_id, levels = c(1, 4), labels = phi_factor_levels)
...
lapply(seq_along(two_dts), function(i){
  ...
  ggsave(file.path("figures", "predictions_world", model_tp, "latest_maps", fl_nms[i]),
         width = 12, 
         height = 4, 
         units = "in")
